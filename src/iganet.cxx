...
int main()
{
  ...
  {
    iganet::IgANet<real_t,iganet::UniformBSpline,5> net({50,30,70}, // Number of neurons per layers
  ...
  {
    iganet::IgANet<real_t,iganet::UniformBSpline,5,5> net({50,30,70}, // Number of neurons per layers
  ...
  {
    iganet::IgANet<real_t,iganet::UniformBSpline,5,5,5> net({50,30,70}, // Number of neurons per layers
  ...
  {
    iganet::IgANet<real_t,iganet::UniformBSpline,5,5,5,5> net({50,30,70}, // Number of neurons per layers
  ...
  {
    std::array<std::vector<real_t>,2> kv =
      {
        {
          {0,0,0,0.2, 0.3, 1,1,1},
          {0,0,0,0.7,1,1,1}
    ...
    iganet::IgANet<real_t,iganet::NonUniformBSpline,2,2> net({50,30,70}, // Number of neurons per layers
                                                          kv);        // Knot vectors
    std::cout << net << std::endl;
    net.sol().transform( [](const std::array<real_t,2> X){ return std::array<real_t,1>{ sin(M_PI*X[0])*sin(M_PI*X[1]) }; } );
    std::cout << net.sol().eval( torch::stack({torch::full({1}, 0.5), torch::full({1}, 0.5)}).flatten() ) << std::endl;
