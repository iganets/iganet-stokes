...
public:
  ...
  loss(const torch::Tensor &outputs,
    ...
    auto bdr_pred =
        Base::outputs_.template eval<iganet::functionspace::boundary>(
            variable_samples.second);
    auto bdr_cond =
        Base::variable_.template eval<iganet::functionspace::boundary>(
            variable_samples.second);
    ...
    // Evaluate pde loss
    auto sol_ilaplace =
        Base::outputs_.ihess(Base::geometry_, variable_samples.first);
