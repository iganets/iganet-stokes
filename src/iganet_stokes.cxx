...
public:
  ...
  torch::Tensor loss(const torch::Tensor &outputs, int64_t epoch) override {
    ...
    // Evaluate the Jacobian operator
    auto u_ijac =
        Base::u_.ijac(Base::G_, collPts_.first, var_knot_indices_,
    ...
    return torch::mse_loss(*u_ijac[0]); //+
...
int main() {
  ...
  using variable_t = iganet::RT2<iganet::S2<iganet::UniformBSpline<real_t, 1, 2, 2>>>;
  ...
  //  return std::array<real_t, 2>{-2.0 * M_PI * M_PI * sin(M_PI * xi[0]) *
  //                               sin(M_PI * xi[1])};
  //});
  //net.f().transform([](const std::array<real_t, 2> xi) {
  //  return std::array<real_t, 1>{0.0*xi[0]+0.0*xi[1]};
  //  });
  ...
  //      return std::array<real_t, 2>{0.0, 0.0};
  //    });
...
//#ifdef IGANET_WITH_MATPLOT
  // Plot the solution
  //TODO extract coefficients for just one component?
