...
public:
  ...
  torch::Tensor loss(const torch::Tensor &outputs, int64_t epoch) override {
    ...
    auto collPts_u_ = std::get<0>(collPts_);
    std::cout << "collPts_u: " << collPts_u_ << std::endl;
    ...
    auto vel = Base::u_.template clone<1, 2>();
    auto p_y = Base::u_.template clone<0, 3>();
    auto p_x = Base::u_.template clone<3, 0>();
    auto vort_x = Base::u_.template clone<0, 3>();
    auto vort_y = Base::u_.template clone<3, 0>();

    // evaluate vorticity at respective collocation points
    auto u_ev_x =vort_x.eval(collPts_u_);
    auto vort_ev_x = u_ev_x;
    //std::cout << "vort_ev_x: " << *vort_ev_x[0] << std::endl;
    //auto f1 = std::get<1>(f);
    //auto f2 = std::get<2>(f);
    //auto f3 = std::get<3>(f);
    //auto vort_ev_x = vort_x.eval(std::get<1>(collPts_.first))[0];
    ...
    auto vel_grad_cpt_p= vel.grad( std::get<3>(collPts_.first) ); // du/dx [ cpt_p ] 
    auto vel_grad_cpt_vort= vel.grad( std::get<0>(collPts_.first) ); // du/dx [ cpt_vort ]
    //std::cout << ", u_grad_x_mass: " << *vel_grad[0] << std::endl; //du/dx
        
    auto p_grad_mom_x = p_x.grad( std::get<1>(collPts_.first) )[0]; // dp [cpt_u]
    auto p_grad_mom_y = p_y.grad(std::get<2>(collPts_.first) )[1]; //dp [cpt_v]
    //std::cout << ", p_grad_mom_x: " << *p_grad_mom_x << std::endl; //dp/dx
    //std::cout << ", p_grad_mom_y: " << *p_grad_mom_y << std::endl; //dp/dy

    auto vort_grad_x = vort_x.grad( std::get<1>(collPts_.first) )[0]; // dvort_x/dx [cpt_u]
    auto vort_grad_y = vort_y.grad( std::get<2>(collPts_.first) )[1]; //dvort_y/dy [cpt_v]
    ...
    // // loss
    //auto res_mom_x = viscosity* *vort_grad_y - vort
    //*p_grad_mom_x - (*vel_hess_mom_x[0]+*vel_hess_mom_x[3]);
    ...
    return torch::mse_loss(*f0[0], *f0[0]); //+
...
int main() {
  ...
  using variable_t = iganet::NE_RT_DG<iganet::UniformBSpline<real_t, 1, 2, 2>,2>;
  ...
  auto& f1 = net.f().template space<0>();
  ...
  auto& f2 = net.f().template space<1>();
  f2.transform([](const std::array<real_t, 2> xi) {
    return std::array<real_t, 1>{
